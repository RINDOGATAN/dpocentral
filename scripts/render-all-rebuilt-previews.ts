 ...
 *
 *   set -a && source .env.local && set +a && npx tsx scripts/render-all-rebuilt-previews.ts [orgNameFilter]
 */
...
import type { ProgramInput } from "../src/server/services/export/privacy-program/data-mapping";
...
async function main() {
...

  {
    ...
    const path = join(OUTPUT_DIR, `Privacy-Program-${safeName}-${fmtDate(now)}.pdf`);
