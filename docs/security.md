...
sibling that is down or slow cannot leave the user signed in here. The list of
siblings is `SUITE_LOGOUT_URLS`, defaulting to the suite on a todo.law host;
the rules are in `src/lib/suite-logout.ts`.
