...
| Public DSAR intake | 5 submissions per 10 minutes per client address; each submission may send a confirmation e-mail | Limiter in the tRPC route handler, matched on the decoded procedure name (the middleware matcher never sees a dotted tRPC path); the e-mail provider's own quota is the outer bound (unknown, to check in the provider console). |
